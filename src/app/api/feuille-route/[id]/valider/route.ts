...
import { keysToCamel } from '@/lib/laravel'
...
export async function POST(
  ...
  try {
    ...
    const res = await fetch(`${LARAVEL_API}/feuille-route/${id}/valider`, {
      method: 'POST',
      headers: {
    ...
    if (!res.ok) {
      return NextResponse.json({ erreur: data.message || 'Erreur lors de la validation' }, { status: res.status })
    }
